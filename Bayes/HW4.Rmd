...
iris_nb <- function(testx, trainx, trainy){
  ...
  temp <- rep(1, N_set) %*% t(x_bar_set)
  var_set <- 1/N_set * t(x_set - temp) %*% (x_set - temp)
  
  x_bar_ver <- colMeans(x_ver)
  temp <- rep(1, N_ver) %*% t(x_bar_ver)
  var_ver <- 1/N_ver * t(x_ver - temp) %*% (x_ver - temp)
  
  x_bar_vir <- colMeans(x_vir)
  temp <- rep(1, N_vir) %*% t(x_bar_vir)
  var_vir <- 1/N_vir * t(x_vir - temp) %*% (x_vir - temp)
  
  like_set <- dmvnorm(testx, mean = x_bar_set, sigma = var_set) * (N_set / N_tot)
  like_ver <- dmvnorm(testx, mean = x_bar_ver, sigma = var_ver) * (N_ver / N_tot)
  like_vir <- dmvnorm(testx, mean = x_bar_vir, sigma = var_vir) * (N_vir / N_tot)
...
While instructive and education (I hope) to write your own NaiveBayes function, in practical vertings, I recommend using the production ready code from some time-tested packages.
...
Task: Write a classifier using the K-nearest neighbors algorithm for the iris data ver.
